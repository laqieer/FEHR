...
void BattlePassiveSkillSEffect(struct BattleUnit* attacker, struct BattleUnit* defender)
{
    ...
    switch(getUnitPassiveSkillB(&defender->unit))
    {
        ...
        case PASSIVE_SKILL_B_MOON_TWIN_WING:
            ...
            break;
        case PASSIVE_SKILL_B_DIVINE_RECREATION:
            if(!(gBattleHitIterator->attributes & BATTLE_HIT_ATTR_FOLLOWUP) && (attacker->hpInitial * 2 >= attacker->unit.maxHp))
            {
...
const struct SpecialSkill specialSkills[] = {
        ...
        {
            "破天",
            "敵の攻撃の２割を奥義ダメージに加算""敵が竜、獣の時、２割ではなく４割を奥義ダメージに加算",
            "Ruptured Sky",
        ...
        {
            "ブルーフレイム",
            "１０を奥義ダメージに加算""味方と隣接している時、さらに１５を奥義ダメージに加算。合計２５を加算する。",
            "Blue Flame",
        ...
        {
            "氷の聖きょう",
            "敵の遠距離攻撃のダメージを３割軽減""奥義発動で軽減した値を、敵に反射",
            "Ice Mirror",
        ...
        {
            "月光閃",
            "敵の守備・魔防８割扱いで攻撃""速さの２割を奥義ダメージに加算",
            "Lunar Flash",
        ...
        {
            "天狼",
            "速さの３割を奥義ダメージに加算""与えたダメージの３割自分を回復",
            "Sirius",
        ...
        {
            "ノヴァの聖戦士",
            "自分から攻撃した時、戦闘後、自分を行動可能\にする（１ターンに１回のみ）""この奥義発動時、自分とダブル相手に移動を最大1マスに制限する状態異常を付与（次回行動終了まで）",
            "Njorun's Zeal",
        ...
        {
            "双刃",
            "魔防の４割を奥義ダメージに加算""奥義以外のスキルによる「ダメージを〇割軽減」を無効",
            "Twin Blades",
        ...
        {
            "開世",
            "守備の半分を奥義ダメージに加算""与えたダメージの４分の１自分を回復",
            "Open the Future",
        ...
        {
            "魔弾",
            "１ターン目開始時、奥義発動カウント－３"TCC_NEWLINE"１５を奥義ダメージに加算、かつ、"TCC_NEWLINE"敵の守備か魔防の低い方でダメージ計算",
            "Seior Shell",
        ...
        {
            "きょうだん",
            "１ターン目開始時、奥義発動カウント－３"TCC_NEWLINE"奥義発動時、"TCC_NEWLINE"敵の守備、魔防半分扱いで攻撃",
            "Brutal Shell",
...
void ComputeBattleUnitPassiveSkillEffects(struct BattleUnit* attacker, struct BattleUnit* defender)
{
    ...
    switch (getUnitPassiveSkillA(&attacker->unit))
    {
        ...
        case PASSIVE_SKILL_A_ATK_SPD_SOLO_4:
            ...
            break;
        case PASSIVE_SKILL_A_ATK_RES_SOLO_1:
            if(!isAdjacentToAnyCompanion(&attacker->unit))
            {
                attacker->battleAttack += 2;
                if((GetItemAttributes(defender->weapon) & IA_MAGICDAMAGE) || (GetItemAttributes(defender->weapon) & IA_MAGIC))
            ...
            break;
        case PASSIVE_SKILL_A_ATK_RES_SOLO_2:
            if(!isAdjacentToAnyCompanion(&attacker->unit))
            {
                attacker->battleAttack += 4;
                if((GetItemAttributes(defender->weapon) & IA_MAGICDAMAGE) || (GetItemAttributes(defender->weapon) & IA_MAGIC))
            ...
            break;
        case PASSIVE_SKILL_A_ATK_RES_SOLO_3:
            if(!isAdjacentToAnyCompanion(&attacker->unit))
            {
                attacker->battleAttack += 6;
                if((GetItemAttributes(defender->weapon) & IA_MAGICDAMAGE) || (GetItemAttributes(defender->weapon) & IA_MAGIC))
            ...
            break;
        case PASSIVE_SKILL_A_ATK_RES_SOLO_4:
            if(!isAdjacentToAnyCompanion(&attacker->unit))
            {
                attacker->battleAttack += 7;
                if((GetItemAttributes(defender->weapon) & IA_MAGICDAMAGE) || (GetItemAttributes(defender->weapon) & IA_MAGIC))
...
const struct PassiveSkill passiveSkillAs[] = {
    ...
    {"攻撃魔防の孤軍１", "周囲１マス以内に味方がいない時、戦闘中、攻撃、魔防＋２", "Atk/Res Solo 1", "If unit is not adjacent to an ally, grants Atk/Res+2 during combat."},
    {"攻撃魔防の孤軍２", "周囲１マス以内に味方がいない時、戦闘中、攻撃、魔防＋４", "Atk/Res Solo 2", "If unit is not adjacent to an ally, grants Atk/Res+4 during combat."},
    {"攻撃魔防の孤軍３", "周囲１マス以内に味方がいない時、戦闘中、攻撃、魔防＋６", "Atk/Res Solo 3", "If unit is not adjacent to an ally, grants Atk/Res+6 during combat."},
    {"攻撃魔防の孤軍４", "周囲１マス以内に味方がいない時、戦闘中、攻撃、魔防＋７", "Atk/Res Solo 4", "If unit is not adjacent to an ally, grants Atk/Res+7 during combat."},
    {"攻撃守備の機先１", "敵のＨＰが全部で戦闘開始時、または、敵が【不利な状態異常】を受けている時、戦闘中、自分の攻撃、守備＋３", "Atk/Def Catch 1", "At start of combat, if foe's HP = 100% or if【Penalty】is active on foe, grants Atk/Def+3 to unit during combat."},
